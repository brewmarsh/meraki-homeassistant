...
async def main():
    ...
    hass.data = {
        "meraki_ha": {
            "test": {
                "client": MagicMock(),
                "coordinators": {
                    "meraki_ha_ssids_test": MagicMock()
                }
            }
        }
    }
    ...
    with patch("homeassistant.helpers.device_registry.async_get", MagicMock()):
        ...
        from custom_components.meraki_ha.sensor import async_setup_entry as sensor_async_setup_entry

        # Mock the coordinator data
        coordinator = MagicMock()
        coordinator.data = {
            "devices": [{"serial": "Q2KX-ACU9-ZAVN", "name": "switch_garage_gs120_8p", "productType": "switch"}],
