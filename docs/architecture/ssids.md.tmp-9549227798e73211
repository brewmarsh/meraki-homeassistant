...
5. **Phase 1: Handler and Service Creation**
...
6. **Phase 2: Updating the Discovery Service**
...
7. **Phase 3: Entity Refactoring and Association**

   - **Update SSID Entities:** Open the existing files for the SSID-related entities (e.g., `custom_components/meraki_ha/sensor/ssid_client_count.py`, `custom_components/meraki_ha/switch/ssid_broadcast.py`).
   - Modify their `__init__` methods to accept a `device_info` object that represents the new virtual SSID device.
   - Update their `device_info` property to use the unique SSID identifier to link the entity to the virtual device instead of the main organization device.

8. **Phase 4: Testing and Validation** - **Create `test_ssid_handler.py`:** Add a new unit test file to thoroughly test the `SSIDHandler`. - Test cases should include: - Verifying that the handler successfully creates a virtual device for each SSID. - Confirming that all SSID-related entities are correctly created and linked to their respective virtual devices. - Asserting that no entities are incorrectly associated with the main organization device. - Execute all tests and perform manual verification within a Home Assistant environment.
