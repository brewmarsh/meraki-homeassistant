...
class MerakiSSIDNameText(CoordinatorEntity[SSIDDeviceCoordinator], TextEntity):
    ...
    def __init__(
        ...
        self.entity_description = EntityDescription(
            ...
            # native_min and native_max removed as they caused TypeError
        )
        # Set these directly on the entity instance
        ...
        self._attr_has_entity_name = True # Let HA combine device name and entity description name
    ...
    def available(self) -> bool:
        ...
        return self._ssid_unique_id in self.coordinator.data and \
               isinstance(self.coordinator.data.get(self._ssid_unique_id), dict) and \
               self.coordinator.data[self._ssid_unique_id].get("enabled", False)
    ...
    def _update_internal_state(self) -> None:
        ...
        if current_ssid_data and isinstance(current_ssid_data, dict):
            ...
            # Update entity name if the SSID name changes in Meraki
            new_base_name = current_ssid_data.get("name", f"SSID {self._ssid_number}")
    ...
    # Add other TextEntity properties if needed (e.g., pattern, min_length, max_length)
    # For now, a simple text field is assumed.
