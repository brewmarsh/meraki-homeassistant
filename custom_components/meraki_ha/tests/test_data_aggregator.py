...
PROCESSED_DEVICES_MR = [
    {"serial": "MR1", "model": "MR52", "networkId": "N1", "tags": ["tag1"], "name": "AP1"},
    {"serial": "MR2", "model": "MR42", "networkId": "N1", "tags": ["tag2"], "name": "AP2"},
]
PROCESSED_DEVICES_MS = [{"serial": "MS1", "model": "MS220", "networkId": "N1", "name": "Switch1"}]
PROCESSED_DEVICES_MX = [{"serial": "MX1", "model": "MX64", "networkId": "N1", "name": "Firewall1"}]
ALL_PROCESSED_DEVICES = PROCESSED_DEVICES_MR + PROCESSED_DEVICES_MS + PROCESSED_DEVICES_MX
...
CLIENT_DATA_N1 = [
    {"id": "C1", "mac": "00:11:22:01", "description": "Client 1", "networkId": "N1", "deviceSerial": "MR1"},
    {"id": "C2", "mac": "00:11:22:02", "description": "Client 2", "networkId": "N1", "deviceSerial": "MR1"},
]
...
def mock_ssid_status_calculator():
    ...
    with patch.object(SsidStatusCalculator, 'calculate_ssid_status', side_effect=mock_calculate_status) as mock_calc:
        yield mock_calc


@pytest.fixture
def aggregator(mock_data_processor, mock_ssid_status_calculator_unused): # mock_ssid_status_calculator_unused is to trigger its patching
    ...
    return DataAggregator(
        ...
        ssid_status_calculator=MagicMock(spec=SsidStatusCalculator) # Pass a mock instance
    )
...
async def test_successful_aggregation(aggregator, mock_ssid_status_calculator):
    ...
    assert len(mr1_aggregated["ssids"]) == len(SSID_DATA_N1) # MR1 is in N1, should get all N1 SSIDs
    ...
    for ssid_in_device in mr1_aggregated["ssids"]:
        original_ssid = next(s for s in SSID_DATA_N1 if s["number"] == ssid_in_device["number"] and s["networkId"] == "N1")
        assert ssid_in_device["name"] == original_ssid["name"]
        assert ssid_in_device["status"] == ("online" if original_ssid["enabled"] else "offline")
    ...
    for ssid in aggregated_data["ssids"]:
        ...
        original_ssid = next(s for s in ALL_SSID_DATA if s["number"] == ssid["number"] and s["networkId"] == ssid["networkId"])
