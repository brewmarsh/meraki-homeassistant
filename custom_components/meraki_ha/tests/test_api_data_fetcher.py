...
async def test_async_get_organization_devices_unexpected_exception(
    ...
    # Changed to ERROR based on typical logging for unexpected
...
async def test_async_get_networks_api_error(
    ...
    assert result is None
    # Message from async_get_networks
    assert "SDK API error during getNetworks" in caplog.text
...
async def test_async_get_mr_device_details_clients_api_error(
    ...
    # Radio settings should still be fetched
    assert device_dict["radio_settings"] == mock_radio_data
...
async def test_async_get_mr_device_details_radio_api_error(
    ...
    # Client count should be fetched
...
async def test_fetch_all_data_ssids_fail_for_one_network(
    ...
    def mock_get_ssids_partial_fail(network_id):
        if network_id == "N_123":
            # Simulate an API error that results in async_get_network_ssids
            # returning None
    ...
    # Expect warnings for the failed SSID fetch
    with caplog.at_level(logging.WARNING):
        all_data = await data_fetcher.fetch_all_data(hass=hass_mock_instance)
...
# If HomeAssistant instance is not actually used by fetch_all_data, this
# can be simplified.
@pytest.fixture
def hass_mock():
