...
This module defines the `MerakiNetworkCoordinator`, which is
responsible for fetching details about all networks within a Meraki
organization. It also registers each Meraki network as a "device" in
the Home Assistant device registry to act as a parent for actual
Meraki hardware devices associated with that network.
"""
import logging
...
# at module load. Ensure MerakiApiDataFetcher and MerakiApiError are
# defined in api_data_fetcher.py.
...
class MerakiNetworkCoordinator(
    ...
    This coordinator retrieves information about all networks in the
    specified Meraki organization. For each network, it creates a
    corresponding "device" entry in the Home Assistant device registry.
    This allows physical Meraki devices to be associated with their
    respective networks in Home Assistant.
    """
    ...
    def __init__(
        ...
        # It's typically passed to DataUpdateCoordinator's constructor or
        # set if needed. For device_registry.async_get_or_create,
        # config_entry_id is on self.
        # self.config_entry is automatically set by Home Assistant when the
        # coordinator is linked to a config entry (e.g. via hass.data).
        # If direct access to ConfigEntry object is needed earlier, it must
        # be passed.
    ...
    async def _async_update_data(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        Retrieves all networks for the configured organization and
        registers each as a device in the Home Assistant device registry.

        Returns:
            A dictionary containing a list of raw network data from the
            API, e.g., `{"networks": [...]}`.

        Raises:
            UpdateFailed: If there's a critical error fetching network
                data or interacting with the device registry.
        """
        ...
        try:
            ...
            # Original code had typo in URL (missing /organizations/)
            url = (
            ...
            if networks_raw is None:
                ...
                _LOGGER.warning(
                    "Failed to retrieve Meraki networks for org %s "
                    "(API fetcher returned None).",
                    self.org_id,
                )
                # Keep self.data as is or clear it, returning current
                # state to avoid entities erroring if they expect data.
                # Or raise UpdateFailed. Original code raised UpdateFailed.
                raise UpdateFailed(
                    f"Failed to retrieve Meraki networks for org "
                    f"{self.org_id} (fetcher returned None)."
                )

            if not networks_raw:  # Handles empty list
                _LOGGER.info(
                    "No Meraki networks found for organization %s.",
                    self.org_id,
                )
            ...
            for network_data in networks_raw:
                ...
                device_registry.async_get_or_create(
                    ...
                    # Generic model for these network "devices"
                    model="Network",
                    # Optional: classify
            ...
            _LOGGER.exception(
                "Unexpected error updating network data for org %s: %s",
                self.org_id,
                e,
            )
            raise UpdateFailed(
