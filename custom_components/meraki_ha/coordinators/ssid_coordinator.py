...
This module defines `MerakiSsidCoordinator`, responsible for fetching
SSID (Service Set Identifier - wireless network name) data for all
relevant Meraki networks. It registers each SSID as a device in the
Home Assistant device registry, linking it to its parent Meraki
network device.
"""
import logging
...
# Local import for MerakiApiDataFetcher to avoid circular dependencies
# at module load.
...
class MerakiSsidCoordinator(DataUpdateCoordinator[Dict[str, List[Dict[str, Any]]]]):
    ...
    This coordinator iterates through networks known to have wireless
    capabilities, fetches their SSIDs, and registers each SSID as a
    distinct "device" in the Home Assistant device registry. These SSID
    devices are parented by their respective Meraki network "devices".
    """
    ...
    def __init__(
        ...
        # This was in __init__ but not used in this class directly
        ...
        Args:
            ...
            device_name_format: The format for naming SSID "devices" in
                the registry (e.g., "prefix", "suffix",
                "network_prefix", "omitted").
        """
        super().__init__(
            hass,
            _LOGGER,
        ...
        # List of network data dicts, expected to be populated by another
        # coordinator or a setup process before _async_update_data is called.
    ...
    async def _async_get_ssids(
        ...
        Args:
            network_id: The ID of the Meraki network for which to fetch
                SSIDs.

        Returns:
            A list of dictionaries, where each dictionary represents an
            SSID. Returns an empty list if the network has no wireless
            SSIDs (e.g., a camera network) or if an error occurs that
            results in no data. Returns None if a critical API error
            occurs preventing data retrieval.

        Raises:
            UpdateFailed: If a critical error occurs during API
                communication, such as invalid API key or unrecoverable
                connection issues.
        """
        url = f"{MERAKI_API_BASE_URL}/networks/{network_id}/wireless/ssids"
        _LOGGER.debug(
            "Fetching SSIDs for network ID: %s from %s", network_id, url
        )

        # Initialize api_fetcher here as it's specific to this request
        # context. Passing 'self' as ssid_coordinator to api_fetcher if
        # it needs context from this coordinator.
        api_fetcher = MerakiApiDataFetcher(
            self.api_key,
            self.org_id,
            network_coordinator=None,
            ssid_coordinator=self,
        )
        ...
        try:
            ...
            if response is None:
                # _fetch_data might return None on certain errors (e.g.
                # non-JSON response, or specific HTTP codes if handled).
                _LOGGER.warning(
                    "API returned no data (None) for SSIDs in network %s. "
                    "This might be normal if the network has no wireless "
                    "capabilities or no SSIDs.",
                    network_id,
                )
                return []  # Treat as no SSIDs for this network

            # Meraki API sometimes returns errors within a 200 OK response,
            # e.g. {"errors": ["..."]}
            if isinstance(response, dict) and "errors" in response:
                ...
                # Depending on how critical this is, you might return []
                # or raise UpdateFailed. If one network failing to provide
                # SSIDs shouldn't stop all, return [].
                return []

            if not isinstance(response, list):
                _LOGGER.warning(
                    "Unexpected API response format (expected list, got %s) "
                    "for SSIDs in network %s: %s",
            ...
            _LOGGER.error(
                "Meraki API error fetching SSIDs for network %s: %s",
                network_id,
                e,
            )
            # If one network's SSIDs failing shouldn't halt everything,
            # return empty list. If it's critical (e.g. auth error),
            # UpdateFailed should be raised by _fetch_data. For now,
            # assume non-critical for individual network SSID fetch unless
            # it's an UpdateFailed type.
            if isinstance(e, UpdateFailed):
                # e.g. ConfigEntryAuthFailed is a subclass
                raise
            ...
            # For truly unexpected errors, re-raise as UpdateFailed if
            # critical for the coordinator. However, if this is part of
            # a loop, you might want to allow other networks to proceed.
    ...
    async def _async_update_data(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        This method iterates through networks determined to have wireless
        capabilities (based on `productTypes`), fetches their SSIDs, and
        registers each SSID as a device in the Home Assistant device
        registry.
        ...
        Raises:
            UpdateFailed: If no network data is available (self.networks
                is empty) or if a critical error occurs during SSID
                fetching or device registration.
        """
        _LOGGER.debug(
        ...
        if not self.networks:
            _LOGGER.warning(
                ...
                "Ensure MerakiNetworkCoordinator runs first or network data "
                "is populated."
            )
            # Return current (likely empty) data to avoid breaking state
            # if minor issue, or raise UpdateFailed if this is considered
            # a critical failure. Original code raised UpdateFailed.
            raise UpdateFailed(
                "Network data not available to MerakiSsidCoordinator."
            )
        ...
        for network in self.networks:
            ...
            if "wireless" not in product_types:
                _LOGGER.debug(
                    "Skipping SSID fetch for network %s (%s) as it does not "
                    "support wireless product types (%s).",
                    network_name,
                    network_id,
                    product_types,
                )
            ...
            _LOGGER.debug(
                "Processing network %s (%s) for SSIDs.",
                network_name,
                network_id,
            )
            try:
                ...
                # If _async_get_ssids raises UpdateFailed (e.g. auth error),
                # propagate it
                _LOGGER.error(
                    "Failed to get SSIDs for network %s due to UpdateFailed: %s",
                    network_id,
                    e,
                )
                raise  # Re-raise to stop the entire update if critical
            except Exception as e:
                # Should be caught by _async_get_ssids, but as a safeguard
                _LOGGER.exception(
                    "Unexpected error from _async_get_ssids for network %s: %s",
                    network_id,
                    e,
                )
            ...
            if network_ssids_raw:
                for ssid_data in network_ssids_raw:
                    ...
                    # Default to True if 'enabled' key is missing, though
                    # API usually provides it.
                    ssid_enabled = ssid_data.get("enabled", True)

                    if ssid_number is None:
                        _LOGGER.warning(
                            "SSID found without a number in network %s, "
                            "skipping: %s",
                            network_id,
                            ssid_data,
                        )
                    ...
                    if self.device_name_format == "prefix":
                        ...
                        formatted_ssid_name = (
                            f"{network_name} - {ssid_name} [SSID]"
                        )
                    # If "omitted", name remains ssid_name

                    # Register SSID as a device in Home Assistant, linked to
                    # the network "device"
                    ...
                    _LOGGER.debug(
                        "Registered/Updated Meraki SSID as HA device: %s "
                        "(Identifier: %s_%s), Enabled: %s",
                        formatted_ssid_name,
                        network_id,
                        ssid_number,
                        ssid_enabled,
