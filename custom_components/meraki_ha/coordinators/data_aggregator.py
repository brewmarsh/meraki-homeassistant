...
combining various processed data streams (devices, SSIDs, networks, tags)
into a single, coherent structure. It utilizes an `SsidStatusCalculator`
to determine the operational status of SSIDs based on device and tag information.
"""
import logging
...
class DataAggregator:
    ...
    This class takes lists of processed devices, SSIDs, networks, and device tags,
    and combines them. A key part of its role is to use the `SsidStatusCalculator`
    to enrich SSID data with their current operational status.
    """
    ...
    def __init__(
        ...
        data_processor: Any, # Replace Any with MerakiDataProcessor if available
        ...
        Args:
            ...
            data_processor: An instance of `MerakiDataProcessor` (or a similar
                class) that provides methods for processing raw data.
                (Currently, this parameter doesn't seem to be directly used in
                `aggregate_data` but is stored).
            ssid_status_calculator: An instance of `SsidStatusCalculator` used
                to determine the status of SSIDs.
        """
        self.relaxed_tag_match: bool = relaxed_tag_match
        self.data_processor: Any = data_processor # Store MerakiDataProcessor instance
    ...
    async def aggregate_data(
        ...
        This method combines the provided lists of devices, SSIDs, and networks
        with device tags. It then uses `SsidStatusCalculator` to update the
        status of each SSID.

        Args:
            processed_devices: A list of dictionaries, where each dictionary
                represents a device that has already been processed (e.g.,
                filtered for relevant types like wireless APs).
            ssid_data: A list of dictionaries, where each dictionary
                represents an SSID.
            network_data: A list of dictionaries, where each dictionary
                represents a network.
            device_tags: A dictionary mapping device serial numbers to a list
                of their tags.

        Returns:
            A dictionary containing the aggregated data, with keys like "devices",
            "ssids" (now including status), "networks", and "device_tags".
            Returns an empty dictionary if a critical error occurs during aggregation.
        """
        _LOGGER.debug(
        ...
        try:
            # Initial aggregation of the provided data
            ...
            combined_data: Dict[str, Any] = {
                "devices": processed_devices,
                "ssids": processed_ssids_with_status, # Use SSIDs with calculated status
                "networks": network_data,
                "device_tags": device_tags,
            }

            _LOGGER.debug(
                "Data aggregation and SSID status calculation complete. Returning combined data."
            )
            ...
            # is equipped to handle it, e.g., raise UpdateFailed(f"Aggregation failed: {e}")
