...
class MerakiApiDataFetcher:
    ...
    async def fetch_all_data(
        ...
        if devices:  # Check if there are devices to process.
            if not isinstance(
                ...
                for (
                    ...
                    _LOGGER.debug(
                        f"Stored firmware map info for {item_serial}: {firmware_info_map[item_serial]}"
                    )
            ...
            for device in devices:
                ...
                if not isinstance(
                    ...
                    _LOGGER.debug(
                        f"Processing device {device_serial} with firmware map info: {info}"
                    )
        ...
        for network_idx, network in enumerate(networks):
            if not isinstance(
                network, dict
            ):  # Should have been caught already if networks was malformed, but as safety
                _LOGGER.warning(
            ...
            try:
                ...
                if network_clients_data:
                    for client_idx, client_data in enumerate(network_clients_data):
                        ...
                        if not mac_address:
                            _LOGGER.warning(
                                "Client data item at index %d for network %s missing 'mac'. Skipping this client: %s",
                                client_idx,
                                network_id,
                                str(client_data)[:100],
                            )
