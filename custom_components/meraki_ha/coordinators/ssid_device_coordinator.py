...
class SSIDDeviceCoordinator(DataUpdateCoordinator[Dict[str, Dict[str, Any]]]):
    ...
    async def _async_update_data(self) -> Dict[str, Dict[str, Any]]:
        ...
        if not isinstance(all_ssids_from_fetcher, list):
            _LOGGER.error(
                "SSID data from main fetcher is not a list (type: %s). Cannot process SSIDs.",
        ...
        for i, ssid_info in enumerate(all_ssids_from_fetcher):
            if not isinstance(ssid_info, dict):
                _LOGGER.warning(
                    "Item at index %d in all_ssids_from_fetcher is not a dictionary, skipping: %s",
                    i,
                    str(ssid_info)[:100],
                )
                continue
            ...
            if not isinstance(is_enabled, bool):
                _LOGGER.warning(
                    "SSID item (Name: %s, Number: %s) 'enabled' flag is not bool (type: %s, value: %s). Defaulting to False for filtering.",
            ...
            if not network_id_raw or not isinstance(network_id_raw, str):
                _LOGGER.warning(
                    "SSID item (Name: %s, Number: %s) has missing or invalid 'networkId' (type: %s, value: %s). Skipping.",
                    ssid_info.get("name", "N/A"),
                    ssid_info.get("number", "N/A"),
                    type(network_id_raw).__name__,
                    str(network_id_raw)[:50],
                )
                continue

            ssid_number_raw = ssid_info.get("number")
            if ssid_number_raw is None:
                _LOGGER.warning(
                    "SSID item (Name: %s, NetworkID: %s) has missing 'number'. Skipping as it's a key identifier.",
                    ssid_info.get("name", "N/A"),
                    network_id_raw,
                )
                continue
            try:
                ...
                _LOGGER.warning(
                    "SSID item (Name: %s, NetworkID: %s) 'number' ('%s') is not a valid integer type. Skipping as it's a key identifier.",
                    ssid_info.get("name", "N/A"),
                    network_id_raw,
