...
class MerakiAPIClient:
    ...
    def _process_initial_data(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        networks_list = networks_res if isinstance(networks_res, list) else []
        if not isinstance(networks_res, list):
            _LOGGER.warning("Could not fetch Meraki networks: %s", networks_res)
        networks: list[MerakiNetwork] = [
            MerakiNetwork.from_dict(n) for n in networks_list
        ]

        devices_list = devices_res if isinstance(devices_res, list) else []
        if not isinstance(devices_res, list):
            _LOGGER.warning("Could not fetch Meraki devices: %s", devices_res)
        devices: list[MerakiDevice] = [
            MerakiDevice.from_dict(d) for d in devices_list
        ]
    ...
    def _build_detail_tasks(
        ...
        for device in devices:
            ...
            elif device.productType == "appliance" and device.networkId:
                ...
                if isinstance(statuses, list):
                    ...
                    prev_device = next(
                        (
                            ...
                            or (isinstance(d, dict) and d.get("serial") == device.serial)
                        ),
                        None,
                    )
                    if prev_device:
