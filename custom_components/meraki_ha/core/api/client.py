...
<<<<<<< HEAD
        ...
        if isinstance(networks_res, Exception):
            ...
            networks = [
                MerakiNetwork.from_dict(network)
                for network in networks_res
            ]
            # organization_id is not in network dict from API usually, but from_dict expects it?
            # Wait, from_dict expects 'organizationId'. Does API return it?
            # Organization networks endpoint returns it.
