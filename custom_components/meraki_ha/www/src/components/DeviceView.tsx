...
const DeviceView: React.FC<DeviceViewProps> = ({ activeView, setActiveView, data }) => {
  ...
  if (!device) {
    return (
      <div>
        <button onClick={() => setActiveView({ view: 'dashboard' })} className="text-blue-500 mb-4">
  ...
  const { name, model, serial, firmware, status, status_messages = [], entities = [] } = device;

  return (
    <div>
      <button onClick={() => setActiveView({ view: 'dashboard' })} className="text-blue-500 mb-4 hover:underline">
        &larr; Back to Dashboard
      </button>
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {entities.map((entity: any) => (
                <tr key={entity.entity_id} className="border-b border-light-border dark:border-dark-border last:border-b-0">
