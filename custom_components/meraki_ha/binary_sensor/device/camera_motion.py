...
class MerakiMotionSensor(MerakiBinarySensor):
    ...
    def is_on(self) -> bool:
        ...
        if not device or not getattr(device, "last_motion_event", None):
            return False

        import time

        last_event = device.last_motion_event
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        if not device:
            return {}
        return {"last_motion_event": getattr(device, "last_motion_event", None)}
