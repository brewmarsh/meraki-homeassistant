...
class RTSPStreamSwitch(MerakiCameraSettingSwitchBase):
    ...
    async def _async_update_setting(self, is_on: bool) -> None:
        """Update the RTSP setting via the Meraki API with optimistic update."""
        ...
        for device in self.coordinator.data.get("devices", []):
            if device.get("serial") == serial:
                ...
                device["video_settings"]["rtspServerEnabled"] = is_on
                break
        ...
        try:
            await self.client.camera.update_camera_video_settings(
                serial=serial, externalRtspEnabled=is_on
            )
            ...
            for device in self.coordinator.data.get("devices", []):
                if device.get("serial") == serial:
                    device["video_settings"]["rtspServerEnabled"] = not is_on
                    break
            self.coordinator.async_update_listeners()
