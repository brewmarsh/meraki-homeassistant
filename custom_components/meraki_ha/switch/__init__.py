...
# Import SSID switch classes
from .meraki_ssid_device_switch import (
    MerakiSSIDEnabledSwitch,
    MerakiSSIDBroadcastSwitch,
)

# Import Camera switch classes
    ...
    if ssid_coordinator and ssid_coordinator.data:
        ...
        for ssid_unique_id, ssid_data in ssid_coordinator.data.items():
            try:
                ...
                _LOGGER.error(f"Error creating SSID switches for {ssid_unique_id}: {e_ssid}", exc_info=True)
    else:
        ...
        for device_info in physical_devices:
            ...
            if product_type == "camera" or model.startswith("MV"):
                ...
                try:
                    ...
                    _LOGGER.error(
                        f"Error creating camera switches for {device_info.get('name', serial)}: {e_cam}",
                        exc_info=True
                    )
    else:
