...
class MerakiPoeUsageSensor(MerakiSensor):
    ...
    def _update_state(self) -> None:
        """Update the internal state and attributes."""
        ...
        for port in ports_statuses:
            if isinstance(port, dict):
                power = port.get("powerUsageInWh", 0) or 0
                total_poe_usage_wh += power
                if "portId" in port:
                    port_id = port["portId"]
                    key = f"port_{port_id}_power_usage_wh"
                    attributes[key] = port.get("powerUsageInWh")
