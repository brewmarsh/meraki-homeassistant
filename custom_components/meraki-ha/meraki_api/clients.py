...
async def get_clients(
    api_key: str, network_id: str, serial_number: str = None
) -> List[Dict[str, Any]]:
    ...
    try:
        _LOGGER.debug("Starting aiohttp session for clients retrieval")
            ...
            async with session.get(url, headers=headers) as response:
                ...
                if serial_number:
                    ...
                    filtered_clients = [
                        client
                        for client in data
                        if client.get("deviceSerial") == serial_number
                    ]
                    _LOGGER.debug(
                        f"Filtered clients by serial number: {filtered_clients}"
                    )
        ...
        _LOGGER.debug("Returning empty list due to aiohttp Client Error")
        return []
    except Exception as err:
        _LOGGER.error(f"An unexpected error occurred: {err}")
        _LOGGER.debug("Returning empty list due to unexpected error")
        return []
