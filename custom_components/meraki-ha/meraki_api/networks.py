...
async def get_meraki_networks(
    api_key: str, organization_id: str, session: aiohttp.ClientSession  # Added session
...
async def get_network_clients_count(
    api_key: str, network_id: str, session: aiohttp.ClientSession  # Added session
) -> Optional[int]:
...
async def get_network_ids_and_names(
    api_key: str, org_id: str, session: aiohttp.ClientSession  # added session
) -> Optional[List[Dict[str, str]]]:
    """Retrieve a list of network IDs and names."""
    networks = await get_meraki_networks(api_key, org_id, session)  # added session
    if networks is None:
        return None
