...
import_pattern = re.compile(
    r"from \.{1,3}core\.api\.client import MerakiAPIClient(?: as ApiClient)?"
)
...
def replace_imports(content, file_path):
    ...
    rel_path = os.path.relpath(
        "custom_components/meraki_ha/core/api", os.path.dirname(file_path)
    )
    rel_path.replace("\\", "/").replace("custom_components/meraki_ha/", "").replace(
        ".", ""
    )
    ...
    depth = (
        len(os.path.relpath(file_path, "custom_components/meraki_ha").split(os.sep)) - 1
    )
    dots = "." * (depth + 1)
