    ...
    "devices": [
        {
            ...
            "ports_statuses": [
                {"portId": "1", "status": "Connected", "enabled": True, "speed": "1 Gbps", "duplex": "full", "usageInKb": {"total": 123456}, "poe": {"enabled": True}, "powerUsageInWh": 10.5},
                {"portId": "2", "status": "Disconnected", "enabled": True, "poe": {"enabled": True}},
                {"portId": "3", "status": "Disconnected", "enabled": True, "poe": {"enabled": True}},
                {"portId": "4", "status": "Disconnected", "enabled": True, "poe": {"enabled": True}},
                {"portId": "5", "status": "Disconnected", "enabled": True, "poe": {"enabled": True}},
                {"portId": "6", "status": "Disconnected", "enabled": True, "poe": {"enabled": True}},
                {"portId": "7", "status": "Disconnected", "enabled": True, "poe": {"enabled": True}},
                {"portId": "8", "status": "Disconnected", "enabled": True, "poe": {"enabled": True}},
            ],
    ...
    with open(os.path.join(WWW_DIR, "screenshot_index.html"), "w") as f:
        f.write("""
            ...
            <head>
                ...
                <style>
                    body { margin: 0; padding: 0; background-color: var(--primary-background-color, #fafafa); }
                </style>
                <script>
                    window.Hass = {
        ...
        page.add_init_script(f"""
            ...
            class HACard extends HTMLElement {{
                ...
                connectedCallback() {{
                    this.shadowRoot.innerHTML = `
                        <style>
                            :host {{ display: block; background: white; border-radius: 4px; box-shadow: 0 2px 2px 0 rgba(0,0,0,0.14), 0 3px 1px -2px rgba(0,0,0,0.12), 0 1px 5px 0 rgba(0,0,0,0.2); color: var(--primary-text-color); margin-bottom: 16px; }}
                            .card-header {{ padding: 16px 16px 0; font-size: 24px; line-height: 32px; color: var(--primary-text-color); }}
            ...
            class HAIcon extends HTMLElement {{
                ...
                connectedCallback() {{
                    ...
                    this.shadowRoot.innerHTML = `<span style="font-family: sans-serif; font-size: 24px;">${{symbol}}</span>`;
                }}
            ...
            class HASwitch extends HTMLElement {{
                constructor() {{ super(); this.attachShadow({{mode: 'open'}}); }}
                connectedCallback() {{
                    this.shadowRoot.innerHTML = `<input type="checkbox" style="transform: scale(1.5); margin: 10px;" />`;
            ...
            document.addEventListener('DOMContentLoaded', () => {{
                ...
                panel.hass = {{
                    states: {{
                        ...
                        "update.lobby_ap_firmware": {{ state: "off", attributes: {{}} }}
                    }},
                    callWS: async (msg) => {{
