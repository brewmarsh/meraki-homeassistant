...
export class MerakiNetworkVitalsCard extends LitElement {
  ...
  private _renderStatusDot(
    ...
    console.log(
      `MERAKI CARD DIAGNOSTIC - Status Dot (${label}) Raw Entity State:`,
      stateObj
    );
    ...
    return html`
      <div
        ...
        @click="${() =>
          isClickable
            ? this._handleEntityClick(entityId, actionConfig)
            : null}"
  ...
  protected render() {
    ...
    if (throughputEntity && this.hass.states[throughputEntity]) {
      console.log(
        ...
        this.hass.states[throughputEntity]
      );
    }

    const throughputStateObj = throughputEntity
      ? this.hass.states[throughputEntity]
      : undefined;
    const throughputState = throughputStateObj
      ? (throughputStateObj.state || '0') +
        ' ' +
        (throughputStateObj.attributes?.unit_of_measurement || '')
      : 'N/A';
