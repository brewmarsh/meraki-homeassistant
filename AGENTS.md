...
## 7. Branch Policy (CRITICAL)

- **Primary Branch:** `beta` is the active development branch.
- **Targeting:** ALL Pull Requests generated by agents MUST target the `beta` branch as the base.
- **Main Branch:** Do NOT target `main` for any fixes or features unless explicitly requested for a production release.
