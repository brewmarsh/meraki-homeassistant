...
async def test_populate_device_entities_picks_camera(coordinator, hass):
    ...
    with patch("custom_components.meraki_ha.meraki_data_coordinator.dr.async_get", return_value=mock_dr), \
         patch("custom_components.meraki_ha.meraki_data_coordinator.er.async_get", return_value=mock_er), \
         patch("custom_components.meraki_ha.meraki_data_coordinator.er.async_entries_for_device", return_value=mock_entries):

        coordinator._populate_device_entities(data)

        device = data["devices"][0]
        print(f"Picked entity_id: {device.get('entity_id')}")
