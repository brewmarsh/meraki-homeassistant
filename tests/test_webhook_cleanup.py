...
async def test_register_webhook_exact_match(network_endpoints, mock_api_client):
    ...
    mock_api_client.organization.get_organization_networks.return_value = [
        {"id": network_id}
    ]

    # Existing webhooks include an exact match
    existing_webhooks = [{"id": "wh1", "name": webhook_name, "url": webhook_url}]

    # Mock the direct API call used in the refactored code
    mock_api_client.dashboard.networks.getNetworkWebhooksHttpServers = AsyncMock(
        return_value=existing_webhooks
    )

    webhook_ids = await network_endpoints.register_webhook(
        webhook_url, "secret", config_entry_id
    )

    assert webhook_ids == ["wh1"]
    # createNetworkWebhooksHttpServer should NOT be called
    assert (
        mock_api_client.dashboard.networks.createNetworkWebhooksHttpServer.call_count
        == 0
    )
...
async def test_register_webhook_cleanup_orphans(network_endpoints, mock_api_client):
    ...
    mock_api_client.dashboard.networks.createNetworkWebhooksHttpServer = AsyncMock(
        return_value={"id": "wh_new"}
    )

    with patch.object(
        network_endpoints, "delete_webhook", new_callable=AsyncMock
    ) as mock_delete:
        webhook_ids = await network_endpoints.register_webhook(
            webhook_url, "secret", config_entry_id
        )
...
async def test_register_webhook_multiple_networks(network_endpoints, mock_api_client):
    ...
    mock_api_client.dashboard.networks.createNetworkWebhooksHttpServer = AsyncMock()
    mock_api_client.dashboard.networks.createNetworkWebhooksHttpServer.side_effect = [
        {"id": "wh_n1"},
        {"id": "wh_n2"},
    ]
